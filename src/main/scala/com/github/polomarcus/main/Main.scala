...
object Main {
  def main(args: Array[String]) {
    ...
    filteredNewsAboutClimate.createOrReplaceTempView("filtered_news")
    val filteredCount = spark.sql("SELECT COUNT(*) FROM filtered_news").as[Long].collect()(0)
    logger.info(s"We have ${filteredCount} news related to climate change")
